...
/**
 * ---------------------------------------------------------
 * EASY TEAM MEMBER VISIBILITY CONFIGURATION
 * ---------------------------------------------------------
 * To show or hide any leadership team member on the /team page:
 * Set their ID to `true` to SHOW them, or `false` to HIDE them.
 * ---------------------------------------------------------
 */
export const SHOW_LEADERSHIP_MEMBERS: Record<string, boolean> = {
  'karthik-b': false, // Karthik B (CEO)
  'prabhu': false,    // Prabhu (CCO)
  'Jaganath': false,  // Jagan (CBO)
  'praneeth': false,  // Praneeth (CRO)
  'vishnu': true,     // Vishnu Varshan (CTO)
  'sushanth': true,   // Sushanth Krushna (CTA)
  'jijin': true,      // Jijin Jose (CSO)
};

/**
 * All leadership definitions (preserved so no profile data is lost).
 */
export const allLeadership = [
  {
    id: 'karthik-b',
...
/**
 * Active leadership roster exported across the site.
 * Automatically filtered based on `SHOW_LEADERSHIP_MEMBERS`.
 */
export const leadership: readonly Person[] = allLeadership.filter(
  (person) => SHOW_LEADERSHIP_MEMBERS[person.id] !== false
);
