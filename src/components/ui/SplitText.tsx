...
export function SplitText({
  ...
  useGSAP(
    () => {
      ...
      // Explicit `gsap.set()` FROM state, rather than letting `gsap.to()` infer
      // it from the CSS class's `transform: translate3d(0, 110%, 0)`. Both
      // approaches usually land the same tween, but inferring "current" yPercent
      // back out of a browser-resolved transform matrix is the kind of thing
      // that can silently go stale — e.g. `gsap.context().revert()` running a
      // second time (React StrictMode's double-invoke, a fast dependency
      // change) restores the *inline* style GSAP last wrote, not the original
      // CSS rule, so a second `gsap.to()` can start from an already-revealed
      // position and never visibly move. Pinning the start value explicitly on
      // every run removes that ambiguity entirely — this is the same pattern
      // `FadeUp` below already uses via `gsap.fromTo`.
