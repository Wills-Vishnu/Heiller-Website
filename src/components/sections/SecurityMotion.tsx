...
import { contact, securityPillars } from '@/lib/site';
import {
  attestations,
  dataLifecycle,
  disclosure,
...
export function SecurityHero() {
  ...
  const meta = [
    { k: 'Framework', v: 'HIPAA · SOC 2' },
    { k: 'Encryption', v: 'AES-256 · TLS 1.2+' },
    { k: 'Tenancy', v: 'Key-isolated' },
...
/**
 * Four attestations, two across, with real padding.
 *
 * Replaces an overlapping fanned stack. The overlap was a nice idea and a bad
 * layout: negative margins pulled each sheet 32px into its neighbour, which
 * came straight out of the text column, and at four across every paragraph was
 * setting to roughly 28 characters.
 *
 * Two-by-two at generous padding gives each entry a ~46 character measure,
 * which is inside the comfortable range for body text. Nothing else on the
 * site uses a plain two-by-two, so the inventory still holds.
 */
export function AttestationGrid() {
  ...
  return (
    <ul ref={ref} className="grid gap-6 lg:grid-cols-2 lg:gap-8">
      {attestations.map((item) => (
        ...
        >
          ...
          <p className="mt-6 max-w-[48ch] text-[1rem] leading-relaxed text-navy/85">
            {item.summary}
          </p>
          <p className="mt-4 max-w-[48ch] text-[0.9375rem] leading-relaxed text-muted">
            {item.detail}
          </p>
        </li>
      ))}
    ...
    <div ref={ref} className="rounded-panel bg-mist/60 p-9 lg:p-14">
      ...
      <div className="mt-14 grid gap-10 border-t border-hairline pt-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div data-cb className="opacity-0">
          ...
          <p className="mt-4 max-w-[54ch] text-[0.9375rem] leading-relaxed text-muted">
            SOC 2 Type II report, penetration test summary, policy set, BAA template
            and completed CAIQ. Under NDA, usually the same business day.
          </p>
